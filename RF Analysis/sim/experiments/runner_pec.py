...
# Only change: copper layers are perfect conductors (see "EXPERIMENT" below). Used to diagnose a
# late-time FDTD instability. Not for production results.
...
def build(model, excite_idx, res, want_ff=False):
    ...
    for c in model["copper_layers"]:
        ...
        if not polys and c["name"] not in port_layers:
            continue
        # EXPERIMENT: copper as a perfect conductor (AddMetal) instead of a finite
        # conductivity sheet, to test whether the sheet model causes the late-time
        # instability. Conductor loss is therefore NOT modelled in this variant.
        prop = csx.AddMetal("cu_" + c["name"])
